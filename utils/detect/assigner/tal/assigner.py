...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes):
        ...
        # 新公式
        # align_metric = torch.exp(bbox_scores.pow(self.alpha) - 1) * overlaps.pow(self.beta)
...
class ExpFreeTaskAlignedAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes):
        ...
        align_metric = bbox_scores.pow(self.alpha) * torch.abs(self.beta * torch.abs(overlaps) / (1 + self.beta - torch.abs(overlaps)))
